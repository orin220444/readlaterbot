...
pub async fn random(cx: UpdateWithCx<Message>){
    ...
    match random_post_opt {
        None => {println!("error while getting random post"); },
        Some(random_post) => {
        cx.answer(/*format!("{:#?}", */&random_post.original_url)
            ...
            ReplyMarkup::InlineKeyboardMarkup(
                InlineKeyboardMarkup::new(
                    vec![
                        vec![
                            InlineKeyboardButton::new("Delete", InlineKeyboardButtonKind::CallbackData("del".into())),
                            InlineKeyboardButton::new("Archive", InlineKeyboardButtonKind::CallbackData("archive".into())),
                        ]
                    ]
                    )
                )
